...
try:
    ...
    raise ImportError(
        "已移除本地 selective_scan 回退逻辑；请确认当前环境已正确安装并可导入 mamba-ssm。"
    ) from e
...
class BSS(nn.Module):
    ...
    def forward_spectral(self, x: torch.Tensor):
        ...
        L = H * W
        K = 2
        xs = torch.cat([x, torch.flip(x, dims=[-2])], dim=1)
...
class SMG(nn.Module):
    ...
    def forward(self, input_spatial: torch.Tensor, input_spectral: torch.Tensor):
        ...
        gate1, gate2 = torch.where(gate1 < self.threshold, torch.zeros_like(gate1), gate1), torch.where(
            gate2 < self.threshold, torch.zeros_like(gate2), gate2)
...
class S2Mamba(nn.Module):
    def __init__(self, patch=11, in_chans=154, num_classes=16, depths=[1], dims=[64], d_state=16, drop_rate=0.,
                 attn_drop_rate=0.0, drop_path_rate=0.1, norm_layer=nn.LayerNorm, **kwargs):
        super().__init__()
