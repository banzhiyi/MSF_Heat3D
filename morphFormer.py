...
class MorphFormer(nn.Module):
    def __init__(self, band, num_classes, patch_size, fm=16, hsi_only=False):
        """
        band: 输入光谱波段数，对应 prepare_dataset 返回的 band
        num_classes: 分类类别数
        patch_size: 空间 patch 尺寸，对应 args.patches
        fm: morphFormer 内部的特征基数，通常和原代码保持 16 即可
        """
        super(MorphFormer, self).__init__()
        global FM, patchsize
        FM = fm          # 若内部代码使用全局 FM，需要同步
        patchsize = patch_size  # 若 CNN.forward 使用全局 patchsize，需要同步

        self.model = CNN(FM, band, num_classes, hsi_only)

    def forward(self, x):
        # x: [B, band, H, W]，与其他模型保持一致
        return self.model(x)
