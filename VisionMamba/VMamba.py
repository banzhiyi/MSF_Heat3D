...
def create_block(
    ...
    # ... 你原来的 if_bimamba / ssm_cfg 处理逻辑保持不变 ...

    factory_kwargs = {"device": device, "dtype": dtype}

    # 1) 先构造“期望传给 Mamba 的参数”
    mamba_kwargs = dict(
        d_state=d_state,
        layer_idx=layer_idx,
        # 下面这些在不同 mamba_ssm 版本里未必存在，所以必须过滤
    ...
    # 2) 过滤掉当前安装的 Mamba 不支持的参数（解决 bimamba_type 报错）
    sig = inspect.signature(Mamba.__init__)
    accepted = set(sig.parameters.keys())
    accepted.discard("self")
    mamba_kwargs = {k: v for k, v in mamba_kwargs.items() if k in accepted}

    mixer_cls = partial(Mamba, **mamba_kwargs)

    # 下面 norm_cls / Block(...) 保持你原来的实现
    norm_cls = partial(
...
class VisionMambaClassifier(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # HSI patch 分类更稳妥的做法：每个像素当作一个 token
        # 因此 patch\_size=1, stride=1，输入空间 img\_size=patches
