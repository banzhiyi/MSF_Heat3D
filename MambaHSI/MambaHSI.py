...
class MambaHSIClassifier(nn.Module):
    """
    将 MambaHSI 的分割输出 (B, C, H, W) 池化成分类输出 (B, C)
    以兼容 `demo.py` 里的训练/验证/测试逻辑。
    """
    def __init__(
        self,
        band: int,
        ...
        if pool == "gap":
            ...
            raise ValueError("pool must be `gap` or `gmp`")

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        logits_map = self.backbone(x)  # (B, num_classes, H, W)
        if self.pool == "gap":
            logits = self.gap(logits_map).squeeze(-1).squeeze(-1)
        else:
            logits = self.gmp(logits_map).squeeze(-1).squeeze(-1)
        return logits
