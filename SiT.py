...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        # 当前项目里，输入 patch 的空间尺寸就是 patch_size
        image_size = patch_size
        window_size2 = patch_size

        # 这里直接用同一个输入构造 Spe/Spa 两路，因此 channels1/channels2 都用 band
        self.core = SiT_Model(
            image_size=image_size,
            patch_size=patch_size,      # 注意：必须能整除 image\_size/window\_size2
            WindowSize2=window_size2,
            dim=dim,
            depth=depth,
            heads=heads,
            head_dim=head_dim,
