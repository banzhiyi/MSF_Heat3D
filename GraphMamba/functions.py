...
def train_and_test_data(pca_image, band, train_point, test_point, true_point, patch, w, h):
    ...
    for i in range(train_point.shape[0]):
        x_train[i, :, :, :], corner_train[i, :], center_pos_train[i] = gain_neighborhood_pixel(pca_image, train_point,
                                                                                               i, patch, w, h)
    for j in range(test_point.shape[0]):
        x_test[j, :, :, :], corner_test[j, :], center_pos_test[j] = gain_neighborhood_pixel(pca_image, test_point, j,
                                                                                            patch, w, h)
    for k in range(true_point.shape[0]):
        x_true[k, :, :, :], corner_true[k, :], center_pos_ture[k] = gain_neighborhood_pixel(pca_image, true_point, k,
                                                                                            patch, w, h)
    ...
    return x_train.reshape(train_point.shape[0], patch * patch, band), x_test.reshape(test_point.shape[0],
                                                                                      patch * patch,
                                                                                      band), x_true.reshape(
        true_point.shape[0], patch * patch,
        band), corner_train, corner_test, corner_true, center_pos_train, center_pos_test, center_pos_ture
...
def load_dataset(Dataset):
    ...
    if Dataset == 'PaviaU':
        uPavia = sio.loadmat('/home/yat/datasets/PaviaU.mat')
        gt_uPavia = sio.loadmat('/home/yat/datasets/PaviaU_gt.mat')
    ...
    if Dataset == 'Salinas':
        SV = sio.loadmat('/home/yat/datasets/Salinas_corrected.mat')
        gt_SV = sio.loadmat('/home/yat/datasets/Salinas_gt.mat')
                ...
                for q in range(l):  # 邻域的列数
                    ...
                    if 0 <= i + (k - (l - 1) / 2) < h and 0 <= (j + (q - (l - 1) / 2)) < w:
                        if abs(k - center) == 0 and abs(q - center) == 0:
                            dis[m, n] = 1
                        if (abs(k - center) == 0 and abs(q - center) == 1) or (
                                abs(k - center) == 1 and abs(q - center) == 0):
                            dis[m, n] = 2
                        if abs(k - center) == 1 and abs(q - center) == 1:
                            dis[m, n] = 3
                        if (abs(k - center) == 0 and abs(q - center) == 2) or (
                                abs(k - center) == 2 and abs(q - center) == 0):
                            dis[m, n] = 4
                        if (abs(k - center) == 2 and abs(q - center) == 1) or (
                                abs(k - center) == 1 and abs(q - center) == 2):
                            dis[m, n] = 5
