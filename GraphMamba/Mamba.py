...
class VisionMamba(nn.Module):
    def __init__(self,
        ...
        for i in range(depth):
            self.layer_GCN.add_module('GCN_Branch' + str(i),
                                      GCN(height=img_size, width=img_size, changel=embed_dim, layers_count=3))
        ...
        for (Mamba, GCN) in zip(self.layers, self.layer_GCN):
            ...
            if nl > 1:
                hidden_states = self.skipcat[nl - 2](
                    torch.cat([hidden_states.unsqueeze(3), last_output[nl - 2].unsqueeze(3)], dim=3)).squeeze(3)
