...
class Attention(nn.Module):
    def __init__(self, dim, heads, dim_head, dropout, dis, D2, edge):
        super().__init__()
    ...
    def forward(self, x, degree, mask=None):
        ...
        #######################中心编码###############################
        # x = x + self.degree_encoder(self.D2)  # 中心编码
        #########################中心编码################################
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_head, dropout, num_channel, mode, dis, D2, edge):
        super().__init__()

        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Residual(PreNorm(dim, Attention(dim, heads=heads, dim_head=dim_head, dropout=dropout, dis=dis, D2=D2,
                                                edge=edge))),
...
class ViT(nn.Module):
    def __init__(self, band, num_token, num_classes, dim, depth, heads, mlp_dim, dis, D2, edge, pool='cls', channels=1,
