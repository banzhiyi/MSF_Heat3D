...
#实现了一个名为VisRetNet的视觉Transformer模型，核心特点是使用了Manhattan Self-Attention(MaSA)机制
...
class GaussianAdaptiveAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        var = x.var(dim=self.norm_axis, keepdim=True) + self.eps
...
#前馈网络，标准的FFN，但增加了可选功能，例如可选的深度卷积DWConv2d增强局部特征
