...
_HAS_MMCV_OPS = False
try:
    from mmcv.ops import (  # type: ignore
        DeformRoIPoolPack,
        DeformRoIPool,
    ...
    DeformRoIPoolPack = None
    DeformRoIPool = None
    ModulatedDeformRoIPoolPack = None
    DeformConv2dPack = None
    warnings.warn(
        f"mmcv.ops not available, deformable ops will be disabled. "
        f"Reason: {type(e).__name__}: {e}"
    )
    _HAS_MMCV_OPS = False

def _require_mmcv_ops(op_name: str) -> None:
    if not _HAS_MMCV_OPS:
        raise RuntimeError(
            f"Requested `{op_name}` but `mmcv.ops` is unavailable "
            f"(missing `mmcv._ext`). Please install a matching `mmcv-full` "
            f"for your PyTorch/CUDA, or switch the model config to avoid deformable ops."
        )
...
try:
    ...
    selective_scan_cuda_core = None  # noqa: N816

try:
    import selective_scan_cuda  # type: ignore
except Exception:
    selective_scan_cuda = None  # noqa: N816
    ...
...
try:
    # triton cross scan（你复制的）一般在这里被 vmamba 引用
    from .csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1
    _HAS_TRITON_CSM = True
except Exception:
    CrossScanTriton = CrossMergeTriton = CrossScanTriton1b1 = None
    _HAS_TRITON_CSM = False
...
class SelectiveScanCore(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1,
        ...
        if selective_scan_cuda_core is not None:
            backend = selective_scan_cuda_core
        elif selective_scan_cuda is not None:
            backend = selective_scan_cuda
        else:
            raise RuntimeError(
                "CUDA selective-scan 扩展未就绪：`selective_scan_cuda_core`/`selective_scan_cuda` 均不可用。"
                "请编译/安装对应扩展，或切换到项目提供的纯 PyTorch 回退实现。"
            )
        out, x, *rest = backend.fwd(
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        if self.ssm_branch:
            if self.post_norm:
                x = input + self.drop_path(
                    self.norm(self.op_spatial(input)))  # + 0*self.drop_path(self.norm(self.op_spectral(input)))
            else:
...
class VSSM(nn.Module):
    def __init__(
        ...
        if DeformRoIPool is None:
            # 可选依赖不可用：禁用 deform pool，避免 NoneType 被调用
            self.deformpool = None
        else:
            self.deformpool = DeformRoIPool(output_size=(7, 7), gamma=0.1)
    ...
    def _make_patch_embed_v2(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm,
        ...
        # v2 仅为 patch\_size=4 设计；其他情况回退到通用版本 `_make_patch_embed`
        if patch_size != 4:
            return VSSM._make_patch_embed(
                in_chans=in_chans,
                embed_dim=embed_dim,
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 假设此时 x 是将要送入 deformpool 的 feature map
        if getattr(self, "deformpool", None) is None:
            # 可选依赖不可用：禁用 DeformRoIPool，直接走 identity 分支
            x1 = x
            # 如果原代码还会用到 x2 / x3 / x4，则同样置为 x，避免后续引用报错
            x2 = x
            x3 = x
            x4 = x
        else:
            # 原来的 RoI 构造逻辑保持不变
            # rois_1, rois_2, rois_3, rois_4 = ...
